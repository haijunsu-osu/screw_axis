...
- the unit direction **s** of the screw axis,
- a point **C** on that axis,
...
Given a rigid-body transform D(x) = A x + t, the Mozzi–Chasles theorem (also known as Chasles’ theorem for kinematics) provides a constructive procedure for the screw parameters:

1. **Rotation axis direction** — The rotation matrix A has a unique unit eigenvector associated with eigenvalue 1. Using quaternion extraction (or a logarithm), we obtain the rotation axis **s** and angle phi.
2. **Parallel/orthogonal translation split** — Decompose the translation vector t into components parallel and orthogonal to **s**: t = (t · **s**) **s** + t_perp. The parallel component yields the axial slide d = t · **s**.
3. **Axis point** — A point **C** on the axis is obtained by solving (I − A) **C** = t_perp together with the constraint **s** · **C** = 0. This ensures **C** lies in the plane perpendicular to **s** through the origin. When t_perp = 0, the axis passes through the origin, so **C** = 0.
4. **Pitch** — The pitch of the screw is p = d / phi (with units of length per radian) when phi is nonzero.
